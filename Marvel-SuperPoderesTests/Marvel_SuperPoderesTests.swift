...
final class Marvel_SuperPoderesTests: XCTestCase {
    ...
    func testRootViewLoadingState() throws {
        let viewModel = RootViewModel()
        let view = RootView().environmentObject(viewModel)
        
        XCTAssertNotNil(view)
        
        let _ = try view.inspect().find(LoadView.self)
    }
    
    func testRootViewLoadedState() throws {
        let viewModel = RootViewModel()
        ...
        let _ = try view.inspect().find(HeroesView.self)
    }
    
    func testRootViewErrorState() throws {
        let viewModel = RootViewModel()
        viewModel.status = .error(error: "Test Error")
        let view = RootView().environmentObject(viewModel)
        
        XCTAssertNotNil(view)
        
        let _ = try view.inspect().find(ErrorView.self)
    }
